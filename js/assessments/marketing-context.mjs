...
export function cleanMarketingContext(input = {}) {
  const context = Object.fromEntries(fields.map(key => [key, typeof input[key] === 'string' ? input[key].trim() : '']));
...
export function marketingContextHTML(input, locked = false) {
  ...
  const text = (key, label, placeholder) => `<label>${label}<input data-marketing-context="${key}" value="${escape(value[key])}" placeholder="${placeholder}"${disabled}></label>`;
