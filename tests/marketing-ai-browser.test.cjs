...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
  ...
  await page.locator('#ai-saving').check();await page.getByRole('button',{name:'See My Review',exact:true}).click();assert.match(await page.locator('#ac-workspace').innerText(),/does not establish sales/);assert.match(await page.locator('#ac-workspace').innerText(),/What you still need to know/);assert.equal(await page.locator('#ac-workspace table tbody tr').count(),8);
  ...
  const comparison=page.locator('summary').filter({hasText:'Human Baseline vs. CEAM+'}).locator('..');await comparison.evaluate(el=>{for(let p=el.parentElement;p;p=p.parentElement)if(p.tagName==='DETAILS')p.open=true;});await comparison.locator('summary').click();
  ...
  await page.getByRole('button',{name:'See My Review',exact:true}).click();
  assert.match(await page.locator('#ac-workspace').innerText(),/Same analysis version, context and initiative/);
  await page.getByRole('link',{name:'Open Dashboard',exact:true}).click();assert.match(await page.locator('#dashboard-assessment-detail').innerText(),/no overall AI score/i);await page.getByRole('link',{name:'Review Report & Action Plan',exact:true}).click();assert.match(await page.locator('#ac-workspace h2').first().innerText(),/AI Use/);
  ...
  await page.getByRole('button',{name:'See My Review',exact:true}).click();
  const stress=page.locator('summary').filter({hasText:'Black Swan / Assumption Stress Test'}).locator('..');await stress.evaluate(el=>{for(let p=el.parentElement;p;p=p.parentElement)if(p.tagName==='DETAILS')p.open=true;});await stress.locator('summary').first().click();assert.match(await stress.innerText(),/Exposed \/ response incomplete/);
