...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
  ...
  await page.locator('[data-ai-field="baseline.text"]').fill('Impressions and content volume do not demonstrate loyalty. Human creative judgment remains important.');assert.equal(await page.locator('#ai-generate').count(),0);await page.getByRole('button',{name:'Lock Human Baseline',exact:true}).click();assert(await page.locator('[data-ai-field="baseline.text"]').getAttribute('readonly')!==null);
